...
extension Shell {
    static func recentTradeListHead() -> UIView {
        RecentTradeListHeadImpl()
    }
}
...
private final class RecentTradeListHeadImpl: UIView {
    ...
    override func didMoveToSuperview() {
        ...
        if !isInstalled {
            ...
            stackView.addArrangedSubview(label(text: "Price(USD)", alignment: .left))
            stackView.addArrangedSubview(label(text: "Qty", alignment: .center))
            stackView.addArrangedSubview(label(text: "Time", alignment: .right))
            stackView.addArrangedSubview(empty(width: 10))
            NSLayoutConstraint.activate([
